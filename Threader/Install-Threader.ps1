<#
.SYNOPSIS
    Installs the Threader Add-in for Autodesk Inventor 2026.

.DESCRIPTION
    This script builds the Threader project and copies the necessary files
...
.EXAMPLE
    .\Install-Threader.ps1 -InstallScope Machine -Configuration Debug
...
$ProjectFile = Join-Path $ProjectDir 'Threader.csproj'
$AddinFile = Join-Path $ProjectDir 'Threader.addin'
...
$TargetFolder = Join-Path $AddinsFolder 'Threader'

# Banner
Write-Host ""
Write-Host "================================================" -ForegroundColor Cyan
Write-Host "  Threader Add-in Installer" -ForegroundColor Cyan
...
$additionalDeps = @(
    'Threader.deps.json',
    'Threader.runtimeconfig.json'
)

# Copy main files
foreach ($file in $filesToCopy) {
    $sourcePath = Join-Path $OutputDir $file
    if ($file -eq 'Threader.addin') {
...
$addinTargetPath = Join-Path $TargetFolder 'Threader.addin'
...
Write-Host "================================================" -ForegroundColor Green
Write-Host ""
Write-Host "The Threader Add-in has been installed to:" -ForegroundColor Cyan
...
Write-Host "  2. Open a Part document"
Write-Host "  3. Go to the 'Power Tools' tab on the ribbon"
Write-Host "  4. Click the 'Threader' button"
Write-Host "  5. Select a cylindrical face to create threads"
Write-Host ""
Write-Host "Keyboard shortcut: Ctrl+L" -ForegroundColor Cyan
Write-Host ""
Write-Host "To uninstall, run: .\Uninstall-Threader.ps1" -ForegroundColor Gray
Write-Host ""
