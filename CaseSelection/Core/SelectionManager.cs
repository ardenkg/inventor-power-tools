...
// SelectionManager.cs - Core Selection Logic with Native SelectSet
// ============================================================================
...
namespace CaseSelection.Core
{
    ...
    /// Selections are appended, not overwritten.
    /// </summary>
    public class SelectionManager : IDisposable
    {
        ...
        public SelectionManager(Inventor.Application inventorApp)
        {
            ...
            _currentFilter = SelectionFilterType.SingleFace; // Default to Single Face
        ...
        public void StartInteraction()
        {
            ...
            try
            {
                ...
                _interactionEvents.StatusBarText = "Class Selection: Click faces to select. Shift+Click to deselect.";
                ...
                _selectEvents.WindowSelectEnabled = true;  // Enable click-and-drag box selection
            ...
            Inventor.View View)
        {
            ...
            // This handler can be used for additional click processing if needed
        }

        /// <summary>
        /// Handles pre-selection (hover) events.
        /// Keep lightweight - only check if object is a Face.
        /// DO NOT run complex topology logic here.
        /// </summary>
            ...
            Inventor.View View)
        {
            ...
            try
            {
                // Only highlight if it's a Face
                if (PreSelectEntity is Face)
                {
                    // Simple check - don't do complex highlighting on hover
                    DoHighlight = true;
                }
            }
            catch
        ...
        /// <summary>
        /// Handles selection events - THIS IS WHERE TOPOLOGY LOGIC RUNS.
        /// </summary>
            ...
            Inventor.View View)
        {
            try
            {
                ...
                // Check if Shift is held (for deselection)
                ...
                // CRITICAL: Clear the SelectEvents selection to allow reselecting the same face
                // Without this, Inventor thinks the face is already selected and won't fire OnSelect again
                if (_selectEvents != null)
                {
                    try
                    {
                ...
                // Update highlights to show selected faces
                UpdateHighlights();
            }
            catch (Exception ex)
            {
        ...
        /// Selections are APPENDED to the existing list, not replaced.
        /// In template mode, adds to template faces instead of main selection.
        /// Shift+Click always deselects. Click on already-selected face toggles it off.
        /// </summary>
        private void ProcessFaceSelection(Face seedFace, bool isDeselecting)
        {
            // Get faces based on current topology filter
            ...
            // Check if the seed face is already in the collection
            // If it is, and we're not explicitly deselecting with Shift, treat as toggle (deselect)
            bool seedAlreadySelected = targetCollection.Contains(seedFace);

            if (isDeselecting || seedAlreadySelected)
            {
                // Remove all faces in the group (Shift+Click OR clicking on already-selected face)
                foreach (var face in facesToProcess)
                {
                    targetCollection.Remove(face);
                }
                System.Diagnostics.Debug.WriteLine($"Deselected {facesToProcess.Count} faces. Mode: {(isDeselecting ? "Shift" : "Toggle")}");
            }
            else
            {
                // APPEND all faces in the group (additive selection)
                foreach (var face in facesToProcess)
                {
            ...
            // CRITICAL: Update highlights after every selection change
            UpdateHighlights();
        ...
        /// Returns true if any faces were cleared, false if already empty.
        ...
        public void SelectAllVisibleFaces()
        {
            try
            {
                ...
                // Get all faces from all bodies and APPEND them
                ...
                {
                    // For assemblies, get faces from occurrence bodies
        ...
        /// <summary>
        /// Adds faces from an occurrence (including nested occurrences).
        /// </summary>
        private void AddFacesFromOccurrence(ComponentOccurrence occurrence)
        {
            try
            {
                ...
                // Recursively process sub-occurrences
        ...
        public void InvertSelection()
        {
            try
            {
                ...
                // Invert: keep only faces NOT in current selection
        ...
        /// Template faces stay orange until Add to Main is pressed.
        /// </summary>
        public void FindSimilarFaces()
        {
            try
            {
                ...
                // Find similar faces and add them to template collection (they stay orange)
                FindSimilarToTemplate();

                // Stay in template mode - don't switch back
                // User presses "Add to Main" to move everything to main selection
        ...
        private void FindSimilarToTemplate()
        {
            ...
            // Get properties of template faces
            ...
            // Search all faces for similar properties
            if (doc is PartDocument partDoc)
            {
                var compDef = partDoc.ComponentDefinition;
                foreach (SurfaceBody body in compDef.SurfaceBodies)
                {
                    foreach (Face face in body.Faces)
                    {
                        ...
                        {
                            _templateFaces.Add(face);  // Add to template (orange) not main
                        }
                    }
                }
            }
        ...
        private FaceProperties GetFaceProperties(Face face)
        {
            ...
            try
            {
                // Get additional geometry-specific properties
        ...
        {
            const double areaTolerance = 0.01; // 1% tolerance
            const double radiusTolerance = 0.001; // 0.1mm tolerance

            foreach (var template in templateProps)
            {
                // Must be same surface type
        ...
        /// Faces change from orange highlight to blue highlight.
        /// </summary>
        public void AddTemplateFacesToMain()
        {
            // Move all template faces to main selection
            foreach (var face in _templateFaces)
            {
                _selectedFaces.Add(face);
            }
            ...
            // Force refresh of highlights - this ensures blue highlights appear
            // Clear template highlight set first, then update main
            if (_templateHighlightSet != null)
            {
                try { _templateHighlightSet.Clear(); } catch { }
            }
        ...
        /// <summary>
        /// Finds all faces in the document that have the specified color.
        /// Adds found faces to the color faces collection (purple highlight).
        /// </summary>
        ...
        /// <summary>
        /// Recursively finds faces matching the target color in an occurrence.
        /// </summary>
        ...
        /// <summary>
        /// Adds color faces to the main selection and clears the color faces list.
        ...
        /// <summary>
        /// Switches to template selection mode. Faces clicked will be added to template list.
        /// </summary>
        ...
        /// <summary>
        /// Commits the current selection to Inventor's native selection set.
        /// This is called when the user presses "Done".
        /// </summary>
        public void CommitSelection()
        {
            try
            {
                // Stop interaction first to cleanup highlight sets
                StopInteraction();

                var doc = _inventorApp.ActiveDocument;
                if (doc == null) return;

                // Clear current Inventor selection and add all our faces
                doc.SelectSet.Clear();
                ...
                foreach (var face in _selectedFaces)
                {
                    ...
                    {
                        // Face might be invalid, skip
                    }
                }

                // Also add template faces if any remain
                foreach (var face in _templateFaces)
                {
                    try
                    {
                ...
                // Also add color faces if any remain
                foreach (var face in _colorFaces)
                {
                    try
                    {
                ...
                System.Diagnostics.Debug.WriteLine($"Committed {_selectedFaces.Count + _templateFaces.Count + _colorFaces.Count} faces to selection.");
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"Error committing selection: {ex.Message}");
            }
        }
