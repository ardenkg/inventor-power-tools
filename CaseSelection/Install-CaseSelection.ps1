<#
.SYNOPSIS
    Installs the CaseSelection Add-in for Autodesk Inventor 2026.
...
$ProjectFile = Join-Path $ProjectDir 'CaseSelection.csproj'
$AddinFile = Join-Path $ProjectDir 'CaseSelection.addin'
...
$TargetFolder = Join-Path $AddinsFolder 'CaseSelection'

# Banner
Write-Host ""
Write-Host "================================================" -ForegroundColor Cyan
Write-Host "  CaseSelection Add-in Installer" -ForegroundColor Cyan
...
$additionalDeps = @(
    'CaseSelection.deps.json',
    'CaseSelection.runtimeconfig.json'
)

# Copy main files
foreach ($file in $filesToCopy) {
    $sourcePath = Join-Path $OutputDir $file
    if ($file -eq 'CaseSelection.addin') {
...
$addinTargetPath = Join-Path $TargetFolder 'CaseSelection.addin'
...
Write-Host "================================================" -ForegroundColor Green
Write-Host ""
Write-Host "The CaseSelection Add-in has been installed to:" -ForegroundColor Cyan
